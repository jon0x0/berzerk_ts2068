...
export function resizeScreen(gfx) {
    ...
    // Small phone viewports need fractional downscaling even without CRT.
    const scale = Math.min(slotW / viewW, slotH / viewH) < 1
        ? Math.max(1 / viewW, Math.min(slotW / viewW, slotH / viewH))
        : Math.max(1, Math.min(fitX, fitY));
