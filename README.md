...
[Play Berzerk](https://jon0x0.github.io/berzerk_ts2068/)

A TS2068 Berzerk cartridge with a cartridge-specific touch interface, running
on a local copy of [TSRun](https://github.com/josef-jelinek/TSRun).

Drag the movable-circle pad, then release and press Fire to shoot in the last
movement direction. Coin, Start and 2 Players are at the top right. Insert one
coin for a one-player game or two coins for two players. Landscape fullscreen
fills the display height. Keyboard: QAOP, Space/Enter fire, C coin, 1/Enter
one-player start, 2 two-player start. Touch follows the active player.

The included cartridge is the approved hybrid-death-speech build. The adapter
handles the cartridge's fire-bit mapping. TSRun source is included at the
repository root; its generic interface is available as tsrun.html. Cartridge
personalization lives under personalizations/berzerk/. See roms/README.md for
upstream ROM provenance. Upstream baseline: 3cfca24, with local phone controls.

Serve locally with `python -m http.server 8769`. The site uses only local
modules, ROMs and cartridge assets; no live upstream module dependency.

## Upstream TSRun documentation
...
- Touch controls - show a circular eight-direction pad and red fire button for
  player 1, including in fullscreen.
...
### Touch controls and cartridge embeds

Enable **Touch controls**, or add `touch=1` to either page's URL. The default
is off; `touch=0` disables it and invalid values leave the default unchanged.
Drag the circular left pad with one thumb and hold the red **FIRE** button
with the other. The pad has eight directions and a neutral centre. Both
controls drive the real TS 2068 player 1 joystick contacts, so the cartridge
must support that joystick (these controls do not simulate keyboard keys).
Touch and a connected player 1 gamepad combine their pressed contacts.

The picture stays unobscured: controls sit beside it in wide viewports and
at the lower left and right below it in portrait viewports. They remain inside
the fullscreen view. **Exit fullscreen** restores the page, including on
browsers where Fullscreen falls back to hiding the page controls. Releasing or
cancelling a touch, disabling the controls, changing fullscreen/layout, or
leaving the page releases touch input. On a small screen, the unfiltered
picture scales down to fit too.

Embed a cartridge using the existing `url` option together with `touch=1`:

```html
<iframe
  title="Play Berzerk on TSRun"
  src="https://your-host.example/TSRun/index.html?touch=1&amp;url=cartridges%2Fberzerk.dck"
...
Host the modified TSRun files and your cartridge at the example paths, or
substitute an encoded cartridge URL with CORS access. The cartridge is not
bundled. Tap **Fullscreen** after loading; browsers require a user gesture,
so fullscreen is intentionally not a startup URL switch. The iframe must be
allowed to enter fullscreen by its containing page. No orientation lock is
requested, so portrait play remains available.

For automated browser checks, serve this directory on port 8000, install
Playwright as a development tool (`npm install --no-save playwright` and
`npx playwright install chromium`), then run `node tests/touch.mjs`. Set
`BROWSER_CHANNEL=msedge` to use an installed Edge instead. The application
itself still needs no build or dependencies. Also check a physical phone for
thumb reach, audio activation, and browser-specific fullscreen behavior.
...
- `touch.js` - optional multitouch player 1 pad and fire controls.
