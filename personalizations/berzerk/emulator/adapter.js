// Berzerk cartridge personalization integration around TSRun's live public modules.
// Upstream: https://github.com/josef-jelinek/TSRun
const upstream=new URL('../../../', import.meta.url).href;
...
async function boot(){
  ...
  const [rom0,rom1,cartridge,vert,frag]=await Promise.all([
    ...
    resource('../assets/berzerk.dck',true),
  ...
  function frame(now){
    ...
    // Berzerk currently decodes fire at bit4; TSRun supplies native bit7.
    for(let i=0;i<2;i++) if(!(joystick[i]&128)) joystick[i]&=~16;
    const player=machine.ram[0x7802]===2?1:0;
    joystick[player]&=touch;
    if(!last)last=now;
  ...
  window.berzerk={
    ...
    release(){touch=255;keys.handleBlur(kbd);},
    ...
    reset(){touch=255;keys.handleBlur(kbd);sound.resetSound(sfx);cpu.resetMachine(machine);},
  };
  window.addEventListener('keydown',event=>{window.berzerk.start();keys.handleKeyDown(kbd,event);});
  window.addEventListener('keyup',event=>keys.handleKeyUp(kbd,event));
  window.addEventListener('blur',()=>window.berzerk.release());
  window.addEventListener('pointerdown',()=>window.berzerk.start());
  window.addEventListener('resize',()=>video.resizeScreen(gfx));
  requestAnimationFrame(frame);
  notify('berzerk-ready');
}
boot().catch(error=>{
  console.error(error);
