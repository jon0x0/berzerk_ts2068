...
import {createTouchJoystick} from "./touch.js";
 ...
 *   joystick:           Uint8Array,
 *   touch:              ReturnType<typeof createTouchJoystick>,
 *   chrome:             HTMLElement[],
 *   screenOnlyClass:    boolean,
...
export function createHost(ui, options) {
    ...
    const host = {
        ...
        joystick,
        touch:              createTouchJoystick(ui.screenSlot, ui.screen),
        chrome,
        screenOnlyClass,
        onKeyDown,
        onKeyUp,
    ...
    applySwitchParamValue(ui.touchToggle, options.query.get("touch") ?? "");
    host.touch.setEnabled(ui.touchToggle.checked);
    ui.touchToggle.onchange = function () {
        host.touch.setEnabled(ui.touchToggle.checked);
        resizeHost(host);
    };
    ...
    host.touch.exit.onclick = function () {
    ...
    new ResizeObserver(function () { resizeHost(host); }).observe(host.touch.view);
...
function onFrame(host, now) {
    ...
    host.joystick[0] &= host.touch.contacts();
    if (host.lastNow === 0) {
        host.lastNow = now;
